...
function M.query.workspaces(space_id)
    local p = string.format('{"get_workspaces":{"space_id":%s}}', M.json._json_opt_int(space_id))
    return M.request(p)
end
...
function M.query.windows(space_id)
    local p = string.format('{"get_windows":{"space_id":%s}}', M.json._json_opt_int(space_id))
    return M.request(p)
end
...
function M.query.window_info(window_id)
    local p = string.format('{"get_window_info":{"window_id":%s}}', M.json._json_str(window_id))
    return M.request(p)
end
...
function M.query.workspace_layouts(space_id, workspace_id)
    local p = string.format(
        ...
        M.json._json_opt_int(space_id),
        M.json._json_opt_int(workspace_id)
    )
    return M.request(p)
end
