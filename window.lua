...
function M.window.next()
	return M.execute('{"Reactor":"next_window"}')
end

---@return boolean|nil
---@return string|nil
function M.window.prev()
	return M.execute('{"Reactor":"prev_window"}')
end
...
function M.window.focus(direction)
	local cmd = string.format('{"Reactor":{"move_focus":%s}}', M._json_str(direction))
...
function M.window.toggle_float()
	return M.execute('{"Reactor":"toggle_window_floating"}')
end

---@return boolean|nil
---@return string|nil
function M.window.toggle_fullscreen()
	return M.execute('{"Reactor":"toggle_fullscreen"}')
end

---@return boolean|nil
---@return string|nil
function M.window.toggle_fullscreen_within_gaps()
	return M.execute('{"Reactor":"toggle_fullscreen_within_gaps"}')
end

---@return boolean|nil
---@return string|nil
function M.window.resize_grow()
	return M.execute('{"Reactor":"resize_window_grow"}')
end

---@return boolean|nil
---@return string|nil
function M.window.resize_shrink()
	return M.execute('{"Reactor":"resize_window_shrink"}')
end
